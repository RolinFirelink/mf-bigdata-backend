...
import com.arg.smart.web.data.utils.BaseProductProductionScaleDataListener;
...
public class BaseProductProductionScaleController {
     ...
     * @throws IOException
     */
    @ApiOperation(value = "基地产品胜场规模数据表-Excel导入", notes = "基地产品胜场规模数据表-Excel导入")
    @PostMapping("/excelUpload")
    public Result<Boolean> excelUpload(@RequestParam("file") MultipartFile file) throws IOException {
        EasyExcel.read(file.getInputStream(), BaseProductProductionScaleExcel.class, new BaseProductProductionScaleDataListener(baseProductProductionScaleService)).sheet().doRead();
